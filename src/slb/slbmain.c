 ...
 *-----------------------------------------------------------------------*/
{
  ...
  if (ptr != name) {
    if ((stricmp(&ptr[1], "o")) || (stricmp(&ptr[1], "asm")) || (stricmp(&ptr[1], "rel"))) {
...
#ifdef QDOS
      if ((strchr("_.", ptr[0]) != (char *)NULL) && (strchr("_.", ptr[-1]) == (char *)NULL)) {
#else
    if ((strchr("/.", ptr[0]) != (char *)NULL) && (strchr("/.", ptr[-1]) == (char *)NULL)) {
#endif
        ptr++;
        break;
      }
     ...
     *---------------------------------------------------------------------*/
{
  ...
  if (filelist == (LISTPTR)NULL)
    filelist = listend;

  if (ptr)
    ptr->node.next = (NODEPTR)listend;
  ptr = listend;
     ...
     *----------------------------------------------------------------------*/
{
  ...
  if (vflag) {
    eprintf("%s", text);
    if (maintflag && libname)
      eprintf(" library '%s'", libname);
    eprintf("\n");
  }
...

{
  ...
  DBG(("MAIN", 0x14, "...finished parsing parameters\nargc=%d, optind=%d, maintflag=%d, analflag=%d", argc, optind, maintflag,
       analflag));
  /*
  ...
  if (rflag || dflag) {
    ...
    if (((libfp = open_file(fullworkname, "rb")) == NULL) || ((newlibfp = open_file(libname, "wb")) == NULL)) {
    ...
    if (libfp && newlibfp && feof(libfp)) {
      ...
      DBG(("MAIN", 0x11, "restore failed: newlibfp=%ld, libfp=%ld, feof(libfp)=%d !", (long)newlibfp, (long)libfp,
           (int)feof(libfp)));
      if (libfp)
        fclose(libfp);
      error(16, fullworkname);
