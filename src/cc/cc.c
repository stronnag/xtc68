 ...
 *--------------------------------------------------------------------------*/
{
  ...
  for (nameptr = file_list.next; nameptr; nameptr = nameptr->next) {
    ...
    for (pass = start_pass; (pass <= last_pass) && (pass < LDPRE_PASS); pass++) {
       ...
       */
      if (pass == ASM_PASS && !dolink && *OUTNAME) {
        strcpy(outname, OUTNAME);
      }
  ...
  if (dolink && *OUTNAME) {
    char tmparg[1024];
    strcpy(tmparg, "-o");
    strcpy(tmparg + 2, OUTNAME);
    add_option(LDPRE_PASS, tmparg);
  }
  ...
  if (!(ret = command_line(&passes[LDPRE_PASS], LD_PASS, &passes[LDPOST_PASS], NULL, NULL))) {
     ...
     *	files at this point
     */
    for (ptrptr = passes[DEL_PASS].p_optbuf.argv; (ptrptr != NULL) && (*ptrptr != NULL); ptrptr++) {
      (void)unlink(*ptrptr);
    }
  } else {
