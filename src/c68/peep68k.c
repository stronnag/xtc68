...
static CODE *block_end P1(CODE *, ip) {
  ...
  while (ip != NIL_CODE && ip->opcode != op_bra && ip->opcode != op_jmp && ip->opcode != op_rts && ip->opcode != op_rte) {
...
static BOOL is_label_used_in_expr P2(EXPR *, ep, LABEL, label) {
  ...
  case en_add:
  case en_sub:
    return (is_label_used_in_expr(ep->v.p[0], label) || is_label_used_in_expr(ep->v.p[1], label));
  case en_uminus:
  case en_cast:
...
static BOOL is_dest_overwritten P2(ADDRESS *, ap, CODE *, ip) {
  ...
  case am_freg:
    for (ip2 = ip->fwd; ip2 != NIL_CODE; ip2 = ip2->fwd) {
      ...
      case op_rts:
      case op_rte:
        if (is_temporary_data_register(ap->preg) || is_temporary_float_register(ap->preg)) {
          /*
      ...
      case op_bhs:
        ...
        result = is_dest_overwritten(ap, ip2) && is_dest_overwritten(ap, find_label(ip2->oper1->u.offset->v.l));
        ip2->opcode = op;
        return result;
      case op_bra:
        op = ip2->opcode;
...
BOOL is_equal_address P2(ADDRESS *, ap1, ADDRESS *, ap2) {
  ...
  case am_indx3:
  case am_indx4:
    return ap1->sreg == ap2->sreg && ap1->preg == ap2->preg && is_equalnode(ap1->u.offset, ap2->u.offset);
  case am_rmask:
  case am_smask:
  ...
  for (ip2 = ip2->back; ip2 != NIL_CODE; ip2 = ip2->back) {
    ...
    default:
      ...
      if (ip2->oper2) {
        ...
        if (is_address_changed(ip->oper1, ip2->oper2) || is_address_changed(ip->oper2, ip2->oper2)) {
          return FALSE;
        }
      ...
      if (ip2->oper1) {
        if (is_address_changed(ip->oper1, ip2->oper1) || is_address_changed(ip->oper2, ip2->oper1)) {
          return FALSE;
        }
 ...
 */
static BOOL is_move_redundant P3(__attribute__((unused)) CODE *, ip, __attribute__((unused)) CODE *, ip2,
                                 __attribute__((unused)) BOOL, memory) {
...
void g_fcode P4(OPCODE, op, ILEN, len, ADDRESS *, ap1, ADDRESS *, ap2) {
  if (ap1 != NIL_ADDRESS && ap1->mode == am_freg && (ap2 == NIL_ADDRESS || ap2->mode == am_freg)) {
    len = IL12;
  }
...
static void peep_pea P1(CODE *, ip) {
  ...
  if (prev->opcode == op_lea && prev->oper2->preg == ip->oper1->preg && is_temporary_address_register(ip->oper1->preg)) {
    prev->opcode = op_pea;
...
static void peep_lea P1(CODE *, ip) {
  ...
  if (next->opcode == op_movea && is_equal_address(ip->oper2, next->oper1) && next->oper2->mode == am_areg &&
...
static void peep_move P1(CODE *, ip) {
   ...
   */
  if (is_equal_address(ip->oper1, ip->oper2) && (ip->oper1->mode != am_areg || ip->length != IL2)) {
    peep_delete(ip);
    return;
  }
  ...
  case am_immed:
    ...
    if (ip->length == IL4) {
       ...
       * unless n == 0
       */
      if (ip->oper2->mode == am_adec && ip->oper2->preg == STACKPTR && (ep->nodetype != en_icon || ep->v.i != 0L)) {
        ip->opcode = op_pea;
    ...
    if (is_icon(ep)) {
       ...
       */
      ip2 = ip->fwd;
      if (ip->length == IL2 && ip->oper2->mode == am_adec && ip2->length == IL2 && ip2->opcode == op_move &&
          ip2->oper1->mode == am_immed && is_icon(ip2->oper1->u.offset) && ip2->oper2->mode == am_adec &&
          ip2->oper2->preg == ip->oper2->preg) {
        ip->length = IL4;
        ep->v.u = (ep->v.u & (UVAL)0xffffL) | (ip2->oper1->u.offset->v.u << 16);
        peep_delete(ip2);
        return;
      }
      /*
...
static void peep_add P1(CODE *, ip) {
  ...
  if ((IVAL)1 <= ep->v.i && ep->v.i <= (IVAL)8) {
    next = ip->fwd;
    if (ip->oper2->mode == am_areg && next && next->opcode == op_move && next->length == (ILEN)ep->v.i &&
        next->oper2->mode == am_adec && next->oper2->preg == ip->oper2->preg && !is_address_used(ip->oper2, next->oper1)) {
      /*
       *       The sequence:
       *               ADD.W #4, A7
       *               MOVE.L <ea>, -(A7)
...
static void peep_and P1(CODE *, ip) {
  ...
  if (prev->opcode != op_move || prev->length != size || !is_equal_address(ip->oper2, prev->oper2) ||
...
static void peep_or P1(CODE *, ip) {
  ...
  if (prev->opcode == op_or && prev->oper1->mode == am_immed && is_icon(prev->oper1->u.offset) &&
      is_equal_address(ip->oper2, prev->oper2)) {
    /*
     *   OR #M, <ea>            =>      OR #(M|N), <ea>
     *   OR #N, <ea>
...
static void peep_clr P1(CODE *, ip) {
  ...
  if ((prev = ip->back) == NIL_CODE || prev->opcode != op_clr || !is_equal_address(ip->oper1, prev->oper1))
    return;
...
static void peep_bxx P1(CODE *, ip) {
  static OPCODE revcond[] = {op_bne, op_beq, op_bge, op_bgt, op_ble, op_blt, op_bls, op_blo, op_bhs, op_bhi};
...
static void peep_ext P1(CODE *, ip) {
  ...
  if (next->oper1 && next->oper1->mode == am_indx2 && next->oper1->sreg == ip->oper1->preg) {
    /*
  ...
  case op_adda:
  case op_suba:
    if ((ip->length == IL4) && ((next->oper2->mode == am_areg) && is_equal_oper(next->oper1, ip->oper1))) {
      /*
  ...
  case op_rol:
  case op_ror:
    if (next->oper1->mode == am_dreg && is_equal_oper(next->oper1, ip->oper1))
      peep_delete(ip);
    break;
  default:
    break;
...
static void peep_bra P1(CODE *, ip) {
   ...
   */
  p = target->back;
  if (p != NIL_CODE && ((p->opcode == op_bra) || (p->opcode == op_jmp) || (p->opcode == op_rts) || (p->opcode == op_rte))) {
    p = block_end(target);
    if (p != NIL_CODE && p != ip) {
      if (ip->fwd) {
        ip->fwd->back = p;
...
static void peep_link P1(CODE *, ip) {
  ...
  if (ip->fwd->opcode == op_unlk && is_equal_oper(ip->oper1, ip->fwd->oper1)) {
    peep_delete(ip->fwd);
    peep_delete(ip);
  }
}
