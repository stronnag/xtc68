...
PRIVATE void put_code P1(const CODE *, ip) {
  ...
  if (ip->oper1 != NIL_ADDRESS) {
    oprintf("\t");
    if (ip->oper2 != NIL_ADDRESS && ip->opcode != op_line && ip->opcode != op_idiv) {
...
PRIVATE void put_storage P1(SYM *, sp) {
  ...
  if (is_static(sp)) {
    oprintf("%s%u\ttimes\t%ld db 0%s", prefix, (unsigned)sp->value.l, size, newline);
  } else {
    oprintf("%s\ttimes\t%ld db 0%s", outlate(nameof(sp)), size, newline);
  }
}
...
PRIVATE void put_start P0(void) {
  oprintf("%s Generated by %s (nasm) %s from %s%s", comment, PROGNAME, VERSION, in_file, newline);
