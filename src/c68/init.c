...
static SIZE initarray P2(TYP *, tp, BOOL, brace_seen) {
  ...
  if (((lastst == tk_sconst) && (is_char(rtp) || rtp->type == bt_uchar || rtp->type == bt_schar)) ||
...
static EXPR *constexpr P1(TYP *, tp) {
  ...
  while ((ep->nodetype == en_cast) &&
         (ep->etp->type == bt_long || ep->etp->type == bt_ulong || ep->etp->type == bt_int32 || ep->etp->type == bt_uint32 ||
          ep->etp->type == bt_pointer32) &&
         (ep->v.p[0]->etp->type == bt_long || ep->v.p[0]->etp->type == bt_ulong || ep->v.p[0]->etp->type == bt_int32 ||
          ep->v.p[0]->etp->type == bt_uint32 || ep->v.p[0]->etp->type == bt_pointer32)) {
    ep->v.p[0]->etp = ep->etp;
    ep = ep->v.p[0];
  }
