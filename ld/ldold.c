...
void halt(int n,...)
{
    ...
    case 9:
        ...
        (void) fprintf (stderr,"(file='%s', position = %ld)\n",
                        currentname,
                        (long)(tell(inp_hnd)-(buf_end-buf_ptr)));
#endif /* QDOS */
    }
    va_end(ap);
    exit(-1);
...
void header_command(void)
{
   ...
   while (in_header_com)
      switch (sy.directive)
      {
         case comment : comment_dir();
                        break;
         case xdef    : xdef_dir(0);
                        break;
         case define  : define_dir();
                        break;
         default      : in_header_com=0;
                        break;
      }
}
...
void sect_command(void)
{
   switch (sy.directive)
   {
      case section : sect_dir();
                     break;
      case org     : org_dir();
                     break;
      case common  : comm_dir();
                     break;
      default      : halt(2);
                     break;
   }
...
void body(void)
{
          ...
          sy.directive==define || sy.directive ==comment)
   {
      switch(sy.directive)
      {
         case data    : data_dir();
                        break;
         case offset  : offset_dir();
                        break;
         case xdef    : xdef_dir(1);
                        break;
         case xref    : xref_dir();
                        break;
         case define  : define_dir();
...
void module(void)
{
   ...
   if (sy.directive!=end)
        halt(2);
   if (lstng_flag) fprintf(list_file,"%-12.12s:",module_name);
   i=0;
   ...
   if (lstng_flag) fprintf(list_file,"\n");
...
void write_prog(void)
{
...
#else
   if ((handle=open(prgm_name,O_CREAT|O_WRONLY|O_BINARY|O_TRUNC,
                    S_IRUSR |S_IWUSR)) == -1)
#endif /* XTC68 */
   {
   ...
   stat.d_datalen = bss_size > 0
                        ? ((bss_size + MIN_DATASPACE+1) & ~1)
                        : MIN_DATASPACE;
       ...
       if( *argv[i] == '-' )
       {
           ...
           switch( toupper( argv[i][1] ))
           {
               ...
               case 'B': /* Could be buffer set */
                   if(!strnicmp(argv[i],"-BUFL",5))
                   { /* Set library buffer size */
                       buf_size = atoi( &argv[i][5] );
                       if( buf_size > 2 && buf_size < 999 )
                           buf_size *= 1024;
                       break;
                   }
                   else if(!strnicmp(argv[i],"-BUFP",5))
                   { /* Set program size */
...
int main(int   argc, char  **argv)
{
...
#else
      mem_size=4*1024*1024; buf_size=2*1024*1024;
#endif
   ...
   if (lstng_flag)  {
      ...
      if (list_file!=stdout) fprintf(list_file,"%s %s %s",_prog_name,_version, _copyright);
   }
