...
PRIVATE char *cc[] = {"ra", "f", "hi", "ls", "cc", "cs", "ne", "eq", "vc", "vs", "pl", "mi", "ge", "lt", "gt", "le"};
    ...
    case SROFF_FLAG:
      ...
      case SROFF_SECTION: /* SECTION */
        ...
        if (!stricmp(IdText, "DATA")) {
          ...
          lprintf(".text\n");
        } else if ((!stricmp(IdText, "UDATA")) || (!stricmp(IdText, "BSS"))) {
          lprintf(".bss\n");
...
int mode, reg;
{
  ...
  case 6:
    d = gword();
    lprintf("%d(a%d,%c%d.%c)", BFIELD(d, 7, 8) | (BTST(d, 7) ? 0xFF00 : 0), reg, BTST(d, 15) ? 'a' : 'd', BFIELD(d, 14, 3),
            BTST(d, 11) ? 'l' : 'w');
    break;
  default:
    switch (reg) {
    ...
    case 3:
      d = gword();
      lprintf("%d(pc,%c%d.%c)", BFIELD(d, 7, 8) | (BTST(d, 7) ? 0xFF00 : 0), BTST(d, 15) ? 'a' : 'd', BFIELD(d, 14, 3),
              BTST(d, 11) ? 'l' : 'w');
      break;
    case 4:
      lprintf("#");
 ...
 *----------------------------------------------------------------------*/
{
  ...
  if ((value == 0xFBFB) || (value != 0xFB07)) {
    ...
    value = (value << 16) + (inchar(libfp) << 8) + (inchar(libfp));
  ...
  while (printpass < 2) {
    ...
    case 0x5:
      if (BFIELD(w, 7, 2) == 3) {
        ...
        lprintf("%sq.%c\t#%d,", BTST(w, 8) ? "sub" : "add", size[BFIELD(w, 7, 2)], ((r1 - 1) & 7) + 1);
        op1((int)m2, (int)r2);
      }
      break;
    case 0x6:
    ...
    case 0xD:
      if ((m2 == 0 || m2 == 1) && m1 > 3 && m1 < 7) {
        OPI(op == 9 ? "subx" : "addx", size[BFIELD(w, 7, 2)]);
        m2 <<= 2;
    ...
    case 0xE:
      if (BFIELD(w, 7, 2) == 3) {
        lprintf("%s%c.w\t", shf[BFIELD(w, 10, 2)], BTST(w, 8) ? 'l' : 'r');
        op1((int)m2, (int)r2);
      } else {
        lprintf("%s%c.%c\t", shf[BFIELD(w, 4, 2)], BTST(w, 8) ? 'l' : 'r', size[BFIELD(w, 7, 2)]);
        if (BTST(w, 5)) {
