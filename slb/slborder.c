    ...
    XDEFPTR t;
{
  ...
  if (Wflag) {
    ...
    if ((fp = (LISTPTR)First_Node((NODEPTR)t->xdef))) {
      if (!Next_Node((NODEPTR)fp)) {
        ...
        for (fp = (LISTPTR)First_Node((NODEPTR)t->xdef); (np = (LISTPTR)Next_Node((NODEPTR)fp));
  ...
  if (Yflag) {
    ...
    if (!t->xdef)
      lprintf("**** NO XDEF's IN THIS MODULE ****");
    for (fp = (LISTPTR)First_Node((NODEPTR)t->xdef); fp; fp = (LISTPTR)Next_Node((NODEPTR)fp)) {
  ...
  for (fp = (LISTPTR)First_Node((NODEPTR)t->xref); fp; fp = (LISTPTR)Next_Node((NODEPTR)fp)) {
    ...
    if (Lflag) {
      ...
      for (np = (LISTPTR)First_Node((NODEPTR)t->xdef); np; np = (LISTPTR)Next_Node((NODEPTR)np)) {
        DBG(("PRINT_DEPENDS", 0x108, "... checking '%s'(%ld) against '%s'(%ld)", np->node.name, (long)np->node.name,
             fp->node.name, (long)fp->node.name));
 ...
 *---------------------------------------------------------------------*/
{
  ...
  if (Lflag) {
    ...
    if (vflag)
      eprintf("Creating dependency lists\n");
  }
  if (Wflag) {
    DBG(("LIBRARY_ANALYSIS", 0x101, "Wflag set - do CROSS-REFERENCE LIST"));
    if (vflag)
      eprintf("Creating XDEF Cross-Reference list\n");
  }
  if (Yflag) {
    DBG(("LIBRARY_ANALYSIS", 0x101, "Wflag set - do CROSS-REFERENCE LIST"));
    if (vflag)
      eprintf("Creating FILE/MODULE Cross-Reference list\n");
  }
...
#ifdef FREE_AT_END
  if (vflag)
    eprintf("Freeing temporary memory areas\n");
  Free_Xdefs();
#endif /* FREE_AT_END */
