...
static void putconst P1(const EXPR *, ep) {
  ...
  case en_str:
    ...
    break;
  default:
    FATAL((__FILE__, "putconst", "illegal constant ep %d", ep->nodetype));
    break;
  }
}
...
static void put_type P1(SYM *, sp) {
  TYP *tp = typeof(sp);

  if (is_object_type(tp)) {
    oprintf("\t.type\t%s,@object%s", outlate(nameof(sp)), newline);
    oprintf("\t.size\t%s,%ld%s", outlate(nameof(sp)), tp->size, newline);
  } else {
...
PRIVATE void put_epilogue P2(SYM *, sp, LABEL, label) {
  put_label(label);
  oprintf("\t.size\t%s,%s%u-%s%s", outlate(nameof(sp)), prefix, label, outlate(nameof(sp)), newline);
}
