...
static void check_sizeof P2 (const EXPR *, ep, SIZE, size)
{
	...
	(ep != NIL_EXPR &&
	 ((ep->nodetype == en_fieldref) ||
	  (!trad_option && (is_sym (ep) && is_function_type (ep->etp)))))) {
...
static
#endif				/* TMS320C30 */
EXPR   *mk_fcon P2 (RVAL *, fp, TYP *, tp)
{
...
static EXPR *addops P0 (void)
{
	...
	if (is_pointer_type (ep1->etp)) {
		...
		(is_same_size (tp_pointer, tp_ulong) ? tp_long : tp_short));
	...
	if (is_arithmetic_type (tp2) && is_arithmetic_type (tp3)) {
		...
		set_referenced_type (tp1,
				     qualify_type (referenced_type (tp1),
						   (QUALIFIER) (referenced_type (tp2)->qual | referenced_type (tp3)->qual)));
#ifndef SYNTAX_CORRECT
	    } else {
		message (WARN_PTRCAST);
#endif /* SYNTAX_CORRECT */
...
EXPR   *transform_assign P4 (EXPR *, ep, const CHAR *, name1, const CHAR *, name2, const CHAR *, name3)
{
	 ...
	 *   F - offset of the LHS
	 */
	size = ((((SIZE) ep0->v.bit.width) << 8) |
		((tp_long->size * 8L) -
		 (SIZE) (ep0->v.bit.offset) - (SIZE) (ep0->v.bit.width)) |
